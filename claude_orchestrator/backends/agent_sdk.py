...
class AgentSdkBackend(BackendAdapter):
    ...
    async def continue_job(self, job: Job, state: ConversationState, context: BackendContext) -> BackendResult:
        runner = self._build_runner()
        checkpoint = state.tool_context.get("checkpoint")
        if not checkpoint:
    ...
    def can_resume(self, job: Job, state: ConversationState) -> bool:
        return bool(state.tool_context.get("checkpoint"))
    ...
    def _normalize_result(
        ...
        tool_context["events"] = payload.get("events", [])
        ...
        for artifact_path in payload.get("artifacts", []):
            artifacts.append(
                ArtifactRecord(
                    ...
                    created_at=utcnow(),
                    kind="workspace_file",
                    path=str(Path(workspace) / artifact_path) if not str(artifact_path).startswith("/") else str(artifact_path),
